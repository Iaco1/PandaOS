...
static nsd_t ns_Table[NS_TYPE_MAX];
...
nsd_t* allocNamespace(int type){
    ...
    will be allocated and added to the active list*/
        ...
        nsd_t* ns =ns_Free_h[type]->n_link->next;
        ns_Free_h[type]->n_link->next=ns->next;
        ns->next->prev=ns_Free_h[type]->n_link;
        list_add_tail(ns, ns_Active_h[type]->n_link);
        return ns;
    }
}
...
bool check_ns_Free(int type, int n){
    ...
    for(int i=0; i<n || list==ns_Free_h[type]->n_link){
        i=i+1;
        list=list->next;
    }
    return i>=n;
...
int addNamespace(pcb_t *p, nsd_t *ns){
    ...
    int type=ns->n_type;
    if(!check_ns_Free(type, p_children+1)) returne false;
    else{
